...
with open("IPTV_ZB.txt", 'w', encoding='utf-8') as file:
    ...
    for result in resultxs:
        channel_name, channel_url = result
        if 'CCTV' not in channel_name or '卫视' not in channel_name or '测试' not in channel_name or '湖南' not in \
                channel_name or '长沙' not in channel_name or '金鹰' not in channel_name or '凤凰' not in channel_name or\
